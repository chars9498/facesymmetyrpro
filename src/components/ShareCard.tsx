  ...
  return (
    <div className="w-[360px] bg-[#050505] p-10 flex flex-col items-center gap-6 relative overflow-hidden border border-white/5">
      ...
      <div className="flex flex-col items-center gap-1 relative z-10">
        <h2 className="text-[14px] font-black text-white uppercase tracking-[0.4em] italic">{t('shareCard.testHeadline')}</h2>
      ...
      <div className="flex flex-col items-center gap-0 relative z-10">
        <div className="text-6xl font-black italic tracking-tighter text-white uppercase leading-none">
          {t('shareCard.top', { percentile: result.percentile })}
        </div>
      ...
      <div className="flex flex-col items-center gap-4 w-full relative z-10">
        <div className="flex flex-col items-center">
          <span className="text-[10px] font-black text-white/30 uppercase tracking-[0.3em] mb-1">{t('shareCard.balanceScore')}</span>
        ...
        <div className="flex flex-col items-center gap-0.5">
          <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">{t('shareCard.mainImbalance')}</span>
          <p className="text-[12px] font-bold text-white/90 text-center leading-tight uppercase tracking-tight">
            {result.primaryImbalanceArea}
          </p>
        </div>
      </div>
      ...
      <div className="mt-2 flex flex-col items-center gap-3 relative z-10 w-full">
        <p className="text-[11px] font-bold text-white/40 italic tracking-wide">{t('shareCard.curiosity')}</p>
        ...
        <div className="w-full px-6 py-2.5 bg-transparent border border-emerald-500/40 rounded-full flex justify-center items-center group active:scale-95 transition-transform">
          <p className="text-[11px] font-black text-emerald-400 uppercase tracking-[0.2em]">{t('shareCard.cta')}</p>
        </div>
  ...
  return (
    <div className="w-[360px] bg-[#0A0A0A] p-8 flex flex-col items-center gap-6 relative overflow-hidden">
      ...
      <div className="flex flex-col items-center gap-1">
        <h2 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] font-mono">{t('common.appName')}</h2>
        <p className="text-[8px] font-bold text-white/20 uppercase tracking-widest">{t('shareCard.twinComparison')}</p>
      </div>
        ...
        <div className="space-y-3">
          <div className="aspect-[3/4] rounded-2xl overflow-hidden border border-white/10 shadow-2xl relative">
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-black/60 backdrop-blur-md rounded-full border border-white/10">
              <span className="text-[8px] font-black text-white uppercase tracking-widest">{t('shareCard.left')}</span>
            </div>
          </div>
        </div>
        <div className="space-y-3">
          <div className="aspect-[3/4] rounded-2xl overflow-hidden border border-white/10 shadow-2xl relative">
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-black/60 backdrop-blur-md rounded-full border border-white/10">
              <span className="text-[8px] font-black text-white uppercase tracking-widest">{t('shareCard.right')}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col items-center gap-3">
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-center">
            ...
            <span className="text-[7px] font-bold text-white/30 uppercase tracking-widest mt-1">{t('shareCard.score')}</span>
          </div>
          <div className="w-px h-8 bg-white/10" />
          <div className="flex flex-col items-center">
            <span className="text-xl font-black italic text-indigo-400 leading-none">{t('shareCard.top', { percentile: result.percentile })}</span>
            <span className="text-[7px] font-bold text-white/30 uppercase tracking-widest mt-1">{t('shareCard.rank')}</span>
          </div>
        </div>
        <p className="text-[10px] text-white/60 text-center leading-relaxed font-medium">
          {t('shareCard.resultText', { percentile: result.percentile })}
        </p>
      </div>

      <div className="mt-2 flex flex-col items-center gap-2">
        <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.15em]">{t('shareCard.cta')}</p>
