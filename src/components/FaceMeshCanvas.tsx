  ...
  useEffect(() => {
    ...
    const draw = () => {
      try {
        ...
        if (!ctx) {
          console.error('[FaceMeshCanvas] Failed to get 2D context');
          return;
        }
        ...
        console.log('[FaceMeshCanvas] Draw triggered', { 
          clientWidth: width, 
          clientHeight: height, 
          naturalWidth, 
          naturalHeight,
        ...
        if (width === 0 || height === 0) {
          console.warn('[FaceMeshCanvas] Image has zero client dimensions, skipping draw');
          return;
        }

        if (!landmarks || landmarks.length === 0) {
          console.warn('[FaceMeshCanvas] No landmarks provided, skipping draw');
        ...
        console.log('[FaceMeshCanvas] Draw completed successfully');
      } catch (err) {
        console.error('[FaceMeshCanvas] Draw error:', err);
      }
    };
